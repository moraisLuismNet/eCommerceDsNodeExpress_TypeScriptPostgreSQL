...
async function createRecord(req: RecordRequest, res: Response<ApiResponse>) {
    try {
        ...
        // Handle image - either from file upload or PhotoName
        let imageRecord: string | null = null;
        if (req.file) {
            imageRecord = `/img/${req.file.filename}`;
        } else if (req.body.PhotoName !== undefined && req.body.PhotoName !== null && req.body.PhotoName.trim() !== '') {
            imageRecord = String(req.body.PhotoName).trim();
        }
...
async function updateRecord(req: RecordRequest, res: Response<ApiResponse>) {
    ...
    try {
        ...
        // Handle image update
        if (req.file) {
            updatedData.ImageRecord = `/img/${req.file.filename}`;
        } else if (req.body.PhotoName !== undefined) {
            updatedData.ImageRecord = String(req.body.PhotoName);
        } else {
            updatedData.ImageRecord = record.ImageRecord;
        }
